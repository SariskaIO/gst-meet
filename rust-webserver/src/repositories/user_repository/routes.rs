...
pub async fn start_recording( 
    ...
    let mut is_recording = &app_state.read().unwrap().is_recording;
    if is_recording {
    ...
    if audio_only { // audio only streaming
        ...
        --recv-video-scale-height={} \
        --room-name={} \
        --send-pipeline='uridecodebin uri=rtmp://streaming-edge-nlb-tcp-bbc5ae3a0d2cb7a3.elb.ap-south-1.amazonaws.com:1935/puc1mr225j4wbqj9/4cghhsdqn5x505ng ! queue ! videoscale ! video/x-raw,width=640,height=360 ! videoconvert ! x264enc speed-preset=ultrafast tune=zerolatency name=video' \
